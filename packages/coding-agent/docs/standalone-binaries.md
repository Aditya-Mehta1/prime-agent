...
Each release also publishes `SHA256SUMS.sigstore.json`, a keyless cosign signature over the checksum inventory. Verifying it proves the release came from this repository's release workflow rather than from whoever served the download; see [Release Security](release-security.md) for the exact command and the pinned signer identity. `prime-agent update` performs this check automatically and refuses any update that fails it.

...
const parseCases: Array<[string, string[], Expected]> = [
	...
	["-v", ["-v"], { version: true }],
	["--version does not suppress later args", ["--version", "--help", "x"], { version: true, help: true, messages: ["x"] }],
	["--help", ["--help"], { help: true }],
	["-h", ["-h"], { help: true }],
	["--print", ["--print"], { print: true }],
	["-p", ["-p"], { print: true }],
	["-p keeps a frontmatter prompt positional", ["-p", FRONTMATTER], { print: true, messages: [FRONTMATTER] }],
	["-p does not swallow later options", ["-p", "--provider", "openai", "hi"], { print: true, provider: "openai", messages: ["hi"] }],
	["--continue", ["--continue"], { continue: true }],
	["-c", ["-c"], { continue: true }],
	["bare --resume", ["--resume"], { resume: true }],
	["bare -r", ["-r"], { resume: true }],
	["--resume with a session path", ["--resume", "/path/to/session.jsonl"], { resume: "/path/to/session.jsonl", messages: [] }],
	["--resume with a windows session path", ["--resume", "C:\\Users\\me\\session.jsonl"], { resume: "C:\\Users\\me\\session.jsonl", messages: [] }],
	["--resume with a relative session path", ["--resume", "sessions/current"], { resume: "sessions/current", messages: [] }],
	["-r with a session id", ["-r", "1234abcd"], { resume: "1234abcd", messages: [] }],
	["--resume=value", ["--resume=1234abcd"], { resume: "1234abcd", messages: [] }],
	["--resume value is an authoritative selector", ["--resume", "fix", "the", "bug"], { resume: "fix", messages: ["the", "bug"] }],
	["--resume -- keeps the picker and a prompt", ["--resume", "--", "continue", "it"], { resume: true, messages: ["continue", "it"] }],
	...
	["--system-prompt accepts a dash-leading value", ["--system-prompt", "- only JSON"], { systemPrompt: "- only JSON" }],
	...
	["repeated --append-system-prompt", ["--append-system-prompt", "A", "--append-system-prompt", "B"], { appendSystemPrompt: ["A", "B"] }],
	["--append-system-prompt accepts frontmatter", ["--append-system-prompt", FRONTMATTER], { appendSystemPrompt: [FRONTMATTER] }],
	...
	["--no-extensions keeps explicit -e", ["--no-extensions", "-e", "a.ts"], { noExtensions: true, extensions: ["a.ts"] }],
	["--skill", ["--skill", "./s"], { skills: ["./s"] }],
	["repeated --skill", ["--skill", "./a", "--skill", "./b"], { skills: ["./a", "./b"] }],
	["--prompt-template", ["--prompt-template", "./one", "--prompt-template", "./two"], { promptTemplates: ["./one", "./two"] }],
	...
	[
		...
		["--autonomous", "--autonomous-gate", "npm test", "--autonomous-gate", "npm run lint", "--autonomous-gate-retries", "2", "--autonomous-gate-timeout-ms", "1000"],
		{ autonomous: true, autonomousGates: ["npm test", "npm run lint"], autonomousGateRetries: 2, autonomousGateTimeoutMs: 1000 },
	],
	[
		"autonomous limit flags",
		["--autonomous", "--autonomous-max-continuations", "20", "--autonomous-max-turns", "80", "--autonomous-max-tokens", "500000", "--autonomous-timeout-ms", "1800000"],
		{ autonomous: true, autonomousMaxContinuations: 20, autonomousMaxTurns: 80, autonomousMaxTokens: 500000, autonomousTimeoutMs: 1800000 },
	],
	[
	...
	["a gate command may start with an unknown short flag", ["--autonomous-gate", "-x npm test"], { autonomousGates: ["-x npm test"] }],
	["--goal", ["--goal", "Write a paper"], { goal: "Write a paper" }],
	["--goal accepts a dash-prefixed objective", ["--goal", "-p"], { goal: "-p", print: undefined }],
	["--goal with --goal-token-budget", ["--goal", "g", "--goal-token-budget", "50000"], { goal: "g", goalTokenBudget: 50000 }],
	["session export needs the internal marker", [INTERNAL_RUNTIME_COMMAND_MARKER, "--export", "s.jsonl"], { export: "s.jsonl" }],
	["model list needs the internal marker", [INTERNAL_RUNTIME_COMMAND_MARKER, "--list-models", "sonnet"], { listModels: "sonnet" }],
	["plain messages", ["hello", "world"], { messages: ["hello", "world"] }],
	["@file arguments", ["@README.md", "@src/main.ts"], { fileArgs: ["README.md", "src/main.ts"] }],
	["mixed messages and file args", ["@f.txt", "explain", "@i.png"], { fileArgs: ["f.txt", "i.png"], messages: ["explain"] }],
	[
		"many flags together",
		["--provider", "anthropic", "--model", "claude-sonnet", "--print", "--thinking", "high", "@prompt.md", "Do it"],
		{ provider: "anthropic", model: "claude-sonnet", print: true, thinking: "high", fileArgs: ["prompt.md"], messages: ["Do it"] },
	],
	["-- keeps a dash-leading prompt positional", ["--", "- weights.pt ..."], { messages: ["- weights.pt ..."] }],
	["-- stops option parsing", ["--", "--provider", "openai"], { provider: undefined, messages: ["--provider", "openai"] }],
	["flags before -- still parse", ["--provider", "openai", "--", "-p", "@file"], { provider: "openai", print: undefined, fileArgs: [], messages: ["-p", "@file"] }],
	["a lone --", ["--"], { messages: [] }],
	["--values are consumed when present", ["--model", "claude-sonnet-4-5", "--fork", "abc"], { model: "claude-sonnet-4-5", fork: "abc" }],
];

/** argv -> one diagnostic the parser must report as a hard error. */
const errorCases: Array<[string, string[], string]> = [
	["--export was removed", ["--export", "session.jsonl"], '--export was removed. Use "prime-agent session export <file> [output]".'],
	["--list-models was removed", ["--list-models", "sonnet"], '--list-models was removed. Use "prime-agent model list [search]".'],
	["invalid thinking level", ["--thinking", "hig"], 'Invalid thinking level "hig". Valid values: off, minimal, low, medium, high, xhigh, max'],
	["invalid --mode", ["--mode", "interactive"], 'Invalid --mode "interactive". Valid values: text, json, rpc, acp, daemon'],
	["removed built-in tools", ["--tools", "read,bash,edit"], "Unknown built-in tool(s): read. Available built-in tools: ipython"],
	...
	["--append-system-prompt does not eat --", ["--append-system-prompt", "--", "run"], "--append-system-prompt requires a value"],
	...
	["--autonomous-gate rejects a long-option value", ["--autonomous-gate", "---run"], "--autonomous-gate requires a value"],
	["--autonomous-gate does not eat another autonomous flag", ["--autonomous-gate", "--autonomous-max-turns", "3"], "--autonomous-gate requires a value"],

...
describe("parseTailscaleArgs", () => {
	...
	it("rejects --json with serve instead of silently serving with human output", () => {
		const parsed = parseTailscaleArgs(["serve", "--port", "3000", "--json"]);
		expect(parsed.kind).toBe("error");
		expect(parsed.kind === "error" && parsed.message).toContain("--json is only supported for status");
		expect(parseTailscaleArgs(["--json", "--port", "3000"]).kind).toBe("error");
	});
...
describe("post-serve verification", () => {
	...
	it("treats a bare https proxy target as port 443", () => {
		const httpsDefault = shimTailscale(
			ONLINE,
			JSON.stringify({ Web: { "milk.ts.net:443": { Handlers: { "/": { Proxy: "https://127.0.0.1" } } } } }),
		);
		process.env.PATH = `${httpsDefault.dir}:${process.env.PATH}`;
		expect(runTailscaleServe(443, false)).toBe(0);
	});
...
describe("status failure diagnostics", () => {
	it("reports unparseable status output as a parse failure", () => {
		const shim = shimTailscale("this is not JSON");
		process.env.PATH = `${shim.dir}:${process.env.PATH}`;
		const probe = probeTailscale();
		expect(probe.cliPath).not.toBeNull();
		expect(probe.error).toContain("unparseable status output");
		expect(runTailscaleStatus()).toBe(1);
		expect(runTailscaleStatus(true)).toBe(1);
	});

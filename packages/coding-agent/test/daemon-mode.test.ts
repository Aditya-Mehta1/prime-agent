...
describe("daemon snapshot transfers", () => {
	...
	it("ENG-4677: lets a retained snapshot finish while a newer generation becomes current", async () => {
		...
		const writeSnapshotBuffer = vi.fn((_client: DaemonSocketClient, buffer: Uint8Array) => {
			...
			if (message.type === "session_snapshot_chunk" && message.snapshotId === firstSnapshotId && message.index === 0) {
				firstChunkStarted();
		...
		expect(
			written.filter((message) => message.type === "session_snapshot_chunk" && message.snapshotId === firstSnapshotId),
		).toHaveLength(2);

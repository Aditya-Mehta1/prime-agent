...
describe("interactive heartbeat management", () => {
	...
	it("handles the daemon no longer knowing a heartbeat", async () => {
		for (const [sid, patch] of [
			["gone-session", undefined],
			["current-session", { heartbeat: null }],
		] as const) {
			const entry = { job: heartbeat({ activeSessionId: sid }), sessionName: "Primary session" };
			const harness = Object.create(InteractiveMode.prototype) as HeartbeatManagementHarness;
			harness.heartbeatCatalog = [entry];
			...
			if (patch) expect(harness.patchConnectionState).toHaveBeenCalledWith(patch);
			else expect(harness.patchConnectionState).not.toHaveBeenCalled();
			expect(harness.refreshHeartbeatCatalog).toHaveBeenCalledOnce();
			expect(harness.applyHeartbeatCatalog).toHaveBeenCalledWith([]);
		}
	});

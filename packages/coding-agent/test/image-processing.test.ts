...
describe("image conversion and resize", () => {
	...
	it.each([
		{ name: "keeps a small PNG untouched", data: TINY_PNG, mimeType: "image/png", maxBytes: 1024 * 1024, limit: 100, resized: false },
		{ name: "keeps a small JPEG untouched", data: TINY_JPEG, mimeType: "image/jpeg", maxBytes: 1024 * 1024, limit: 100, resized: false },
		{ name: "shrinks a PNG past the dimension limit", data: MEDIUM_PNG_100x100, mimeType: "image/png", maxBytes: 1024 * 1024, limit: 50, resized: true },
		{
		...
		const result = await resizeImage({ type: "image", data, mimeType }, { maxWidth: limit, maxHeight: limit, maxBytes });

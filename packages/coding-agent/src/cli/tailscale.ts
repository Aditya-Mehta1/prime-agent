 ...
 *
 * This module is the detection core of the Tailscale agent mesh: probeTailscale()
 * is the seam the mesh builds on (tailnet peer discovery, remote sessions labeled
 * with their tailscale connection, cross-machine messaging/spawn). See
 * docs/tailscale.md for the mesh overview.
 *
 * One command group, plus doctor:
 ...
 * Exported as the detection seam for the Tailscale agent mesh (peer discovery,
 * remote-session labels, cross-machine messaging/spawn) and for tests; it is
 * not part of the package's public API (src/index.ts).
 */
...
export function parseTailscaleArgs(args: string[]): TailscaleArgs {
	...
	if (
		json &&
		(rest.includes("serve") ||
			rest.includes("--funnel") ||
			rest.some((arg) => arg === "--port" || arg.startsWith("--port=")))
	) {
		return { kind: "error", message: "tailscale: --json is only supported for status" };
	}

...
/**
 * Settle a pending socket connection: resolve on connect, reject on error, and destroy the
 * socket on timeout. `onFailure` drops the caller's socket reference, so a failed connect
 * never leaves the client holding a socket it never connected.
 */

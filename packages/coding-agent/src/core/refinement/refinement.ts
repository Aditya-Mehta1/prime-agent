...
export function formatHarnessStateForPrompt(
	...
	for (const kind of Object.keys(state.entries) as RefinementKind[]) {
		...
		// within the kind rather than across unrelated kinds. Package overlays
		// rank above their own-kind competition (the package's provenance wins ties).
		...
		const entries = ranked.sort((a, b) => {
			...
			if (packageRank !== 0) return packageRank;

...
async function ensureKernelPythonUncached(
	...
	if (override) {
		...
		if (!(await hasPrimeAgentRuntime(python))) {
			missing.push(
				"a current prime-agent-runtime with callable rlm.spawn, rlm.create_session, rlm.host_request, rlm.progress_note, rlm.rename, and explicit harness CRUD methods",
			);
		}
		if (missing.length === 0) {

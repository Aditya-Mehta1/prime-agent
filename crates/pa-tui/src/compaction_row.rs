...
/// `CompactionSummaryMessageComponent`, collapsed): the header line in
/// `refinementHeader`, the summary below in `refinementSummary` —
/// whitespace collapsed, wrapped at one column of inset, capped at two
/// lines with an ellipsis. `expanded` adds the markdown summary and the
/// dim `Compacted from N tokens` metadata row (the TS component's
/// expansion; this surface renders the collapsed default).
pub fn render_compaction_summary(
    ...
    // The expanded view: full markdown in the summary color, then the dim
    // token metadata with the optional focus.
    let content_width = width.saturating_sub(2).max(1);
    let mut md = crate::markdown::MarkdownStyle::from_theme(theme);
    md.body = body;
    for line in crate::markdown::render_markdown(summary.trim(), content_width, &md) {
...
mod tests {
    ...
    fn summary_row_expanded_metadata() {
        ...
        assert!(
            text.iter()
                .any(|row| row.trim() == "Compacted from 1,234 tokens \u{b7} focus: tests"),
            "{text:?}"
        );
    }

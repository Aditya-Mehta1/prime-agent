...
## Prompt stash across session switches (prompt-stash lane, TS `prompt-stash-state.ts`)

TS ground truth read in full: `prompt-stash-state.ts` (38 lines) + every interactive-mode call site (constructor bind ~1198-1306, hydrate, `stashDraftForAgentsView` ~4452, `restorePromptStashOnOpen` ~4443, `restorePromptStashIfEditorEmpty` ~4481, `retainSubmittedDraft`/`retainStartupPromptDrafts`, the `returnToAgentsView` exit ~7300, `handleResumeSession` ~8973, the TS test file `interactive-mode-prompt-stash.test.ts`).

- The store is CLIENT-owned and per-TUI-PROCESS (TS `main.ts` creates one `ClientPromptStashStore`; the agents-view loop passes it to every `InteractiveMode`). It never persists to disk; a fresh process starts with no stash.
- `PromptStash` = draft text (paste/image markers intact) + `expandedText` + `pasteSnapshot` (TS editor paste markers; the Rust editor keeps the expanded text inline, so the Rust stash carries text + images only) + `images` (marker-id → image bytes referenced by the draft) + `restoreOnOpen`.
- Capture (TS `stashDraftForAgentsView`, only on `returnToAgentsView` — agents-back is empty-editor-gated, `/resume` typed as a command clears the editor, so the live-draft surfaces are the scoped-agents-view open (summary line focused) and the user-rebound `app.session.resume` key, which TS fires with text in the editor): whitespace-only drafts are skipped; the snapshot becomes the session's head with `restoreOnOpen: true`; an existing unrestored head + its queue line up BEHIND it (queue semantics for multi-switch chains).
- Restore (TS `restorePromptStashOnOpen` at `run()` start): only the head, only when the editor is empty, only when `restoreOnOpen`; the restore pops the head and promotes the queue's first draft to the head. The notice "Restored stashed prompt" lands in its own status block (TS clears the status anchor first so a startup notice is not rewritten in place). The draft is placed back in the editor SILENTLY apart from that status row — no other indicator.
- Bind (TS `bindPromptStashSession` on `applyConnectionStateSnapshot`): the stash state follows the session's stable id; re-hydrate re-inserts the stashed images into the paste registry and reserves their marker ids (TS `hydratePromptStash`), so a restored draft's `[image #N]` markers resolve to the stashed bytes on submit.
- Release (TS `release`, from `teardownSessionUi` and rebinding): an empty state drops from the store; a session holding a draft keeps it for the next view that binds it.
- TS behaviors NOT ported in this lane (out of scope, note for the reviewer): the manual ctrl+s stash key (`app.prompt.stash`: "Stashed prompt"/"No prompt to stash"/"Prompt stash already has a draft" + the restore-after-submit and the `retainSubmittedDraft`/`retainStartupPromptDrafts` submission-retention machinery with generation ordering). The Rust surface covers the session-switch contract only.

Rust divergences ruled during the port (each driven by an architecture difference, kept minimal):
- TS has no in-place `/switch`; the Rust `/switch` (same-process reattach) captures the outgoing draft as that session's restore head and CLEARS the editor (TS in-place switches clear the editor via `resetCurrentSessionRenderState`), and the switch back restores — the Kevin directive's "capture on every session-switch path".
- TS `/tree` navigation keeps the draft in the editor (`renderTreeNavigation` never clears it) — nothing to stash; the Rust port matches (no capture on `/tree`).
- TS `/resume <selector>` switches IN PLACE and loses the draft (editor cleared, no capture). The Rust build hands off to a fresh chat run for the same switch; the exit path stashes the draft (Kevin directive: capture on `/resume`). The handoff reopen restores it — Rust diverges from TS here by preserving the draft.
- TS `app.session.resume` action existed but had no Rust handler; ported (same `requestAgentsView` semantics, including the "The agents view needs a daemon-hosted session..." status for `--no-session` runs).
- The clipboard image paste path has a verification seam (`PRIME_AGENT_TEST_CLIPBOARD_IMAGE`, the `script_path` pattern): a fixture file stands in for the clipboard on a headless harness; no display server exists there. The seam sits in `read_clipboard_image` only.
- Cross-surface finding (f24 run 20260921T194517Z, attributed to the agents-view surface, not fixed here): after a chat round trip the TS binary (0.9.5) shows the agents-view search box CLEARED while the Rust build keeps the query (`run_agents_view_flow` carries `view.query`); the TS repo source seeds `persistentState.query` back into the view (`agents-view-mode.ts` line ~788), so the binary and the repo source disagree — the battery diff (frame-diff-agents-view-back.txt) is the evidence either way.

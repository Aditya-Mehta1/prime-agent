...
right before assembly — network fetch of the live catalog repo in CI, a local
catalog checkout via `--catalog-dir`, or an offline `--fixture` snapshot that
still passes the full validation gates (models `schemaVersion == 1` with
≥ 42 distinct `(provider, api, baseUrl)` transport tuples; plugins
`version == 2` with ≥ 68 services). The packer (`assemble_artifacts.py`,
`package_release.py`) and the verifier (`verify_release.py`) share the same
validation functions and hard-fail without validated assets.
...
- `make release-dry-run [RUNTIME_DIR=<path>] [VERSION=<x.y.z>]` — the release verifier,
  ...
  2. Generate the bundled catalog assets (offline `--fixture` by default; pass
     `CATALOG_ASSETS_MODE=network` to mirror the CI fetch).
...
- `make catalog-assets` — generate the bundled catalog assets from the live
  catalog repo (CI parity); `make catalog-assets-fixture` — the offline
  synthetic snapshot; `make catalog-assets-gates` — the bundler/packer test

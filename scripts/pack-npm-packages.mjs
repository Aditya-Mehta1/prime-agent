 ...
 *                        Every executableSha256 must match, so the npm artifact and the R2 artifact
 *                        assert the same hash. Mismatch is a hard error.
...
function collectBinaries(binaryDir, receiptsFile) {
	...
	for (const platform of Object.keys(PLATFORMS)) {
		...
		if (declaredReceipt && declaredReceipt.executableSha256 !== executableSha256) {
			throw new Error(
				`Receipt mismatch for ${platform}: ${receiptsFile} records ${declaredReceipt.executableSha256}, binary hashes ${executableSha256}`,
			);
		}
		binaries.push({
			platform,
			executableSha256,
			...(declaredReceipt ? { file: declaredReceipt.file, sha256: declaredReceipt.sha256 } : {}),
		});
	}
	return binaries;
}

...
- Windows-readiness (not shipping Windows now, but never painting ourselves out): no hard POSIX assumptions anywhere. All platform-specific code lives behind small platform traits in cfg-gated modules - transport (session/daemon sockets as a trait so named-pipes or AF_UNIX can slot in), process control (detached spawn/kill semantics), file locking (flock vs LockFileEx), file permissions (0600 vs ACLs), temp/config dir resolution, and shell selection for the bash tool. Windows support later should mean implementing traits, never re-plumbing. The daemon redesign must define its transport as a trait from day one.
...
- The orchestrator does not implement. The main session decomposes, writes precise lane prompts (each with its verifier), spawns, monitors, reviews, runs merge gates, and reports - it never writes product code itself. Implementation depth lives in the subagent lanes (at max effort); the orchestrator's turns stay short (spawn/collect/merge operations only) so the mission is never blocked behind a long build, a long reasoning stream, or a stuck turn. If a lane is stuck, respawn it from persisted state - do not absorb its work into the main session.
